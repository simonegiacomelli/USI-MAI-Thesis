...
\end{enumerate}
The first issue was solved with using two different intertwined phases of query: probe and harvest. The probe requested only the number of the repositories that were created in a specific time window; if the number exceeds the 1000 limit, it iteratively divides the probe query in two sub-queries using half of the time window for each and add them to the job queue.
...
The program can recover from a crash because every query is cached to a file; when a query is issued and the cache is present the network request is skipped and the file content is used instead. This was important because the URLs mining execution took roughly 120 hours and a system to reuse past expended resources was needed.

The URLs mining tool creates two text files: one with the complete list and one with the URLs that matches our accept criteria. Both files contains the following columns:
...
The total number of URLs retrieved was roughly seven million but we did accept only a subset of them: around 250 thousand repositories.
...
The problems related to the Android OS was that there were more than 1,700 forks/clones and those repositories counted between 300,000 and 550,000 commits; it is one of the biggest project encountered in this endeavour and it posed a big bottleneck to the commit history processing: traversing the commit tree could take a couple of days only for one Android OS repository.
...
This section describes the task of traversing all the commits of a repository and collecting the code snippets viable to be part our SATD/fixed dataset.

The processing is composed by many small pieces. We give an overview and later on add explanations and details.
...
For each repository we loop on every commit and every file change; but we care only for \textit{modify} type operations; we ignore the rest of git file change operations: add, delete, rename and copy.
...
\begin{itemize}
    \item The appearance order of the methods in the source code do not affect the process.
    \item When a method is removed it is automatically excluded through the lack of pairing between the two lists.
    \item The renamed methods are lost; for all intents and purposes they are treated as removed.
\end{itemize}
...
To detect the SATD, we use keyword label techniques using the 62 pattern reported by Potdar and Shihab \cite{potdar2014exploratory} (they are actually 63 \footnote{http://users.encs.concordia.ca/~eshihab/data/ICSME2014/satd.html}). Out of these patterns we create regular expressions that are applied on all the source comments extracted with JavaParser.
...
In our research we found that many repositories were duplicates or clones (GitHub keep track of forks but do not track duplicates). 
We need to be very careful to not introduce noise in the dataset. To ensure a better quality for our samples we implemented the following steps:
\begin{itemize}
    ...
    \item Two additional hashes are calculated: one for the clean before image and a second for the clean after image. Then we discard all pair that are found having an hash in common between any before and after clean image hash; this ensures that we cannot have the same snippet in both before and after image.
