...
\begin{itemize}
    \item \textbf{RQ$_1$} \textit{How does our approach perform in detecting technical debt in methods source code?} This RQ investigates the accuracy of the classifier in identifying technical debt in our dataset.
...
\begin{itemize}
    ...
    \item The verbatim bodies involved in the change: 
    \begin{itemize}
    \item Before the commit, affected with SATD. 
    \item After the commit, not affected with SATD, i.e. fixed.
    \end{itemize}
\item The `cleaned' version of the bodies from the point above (see \ref{sec:cleaning} for details).
\end{itemize}
...
\begin{itemize}
    ...
    \item Methods containing only one instruction being throw exception statement.
...
The training was conducted using 20 epochs; the test performance results were measured on the best epoch as reported by the validation set highest accuracy. We conducted a distributed experiment for increasing the performance through hyperparameter tuning.

To answer RQ$_2$ we tracked all confidence level for each prediction, then we report the metrics value for the following confidence thresholds: any, 0.5, 0.6, 0.7, 0.8 and 0.9. For each of them we compare and comment the precision, recall and F1-score.
